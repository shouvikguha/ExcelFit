...
from manage_members import add_member, verify_member
import re
...
def check_count_on_word(question, num):
    while True:
        try:
            ans = input(question + " ").strip()
            ans_mod = ans.title()
            ...
            if len(words) < num:
                print(f"You must enter items separated by at least {num - 1} commas for this response.")
                continue
            ...
            if contains_num:
                print("Numeric values within words are not allowed. Please enter words only.")
                continue

            if len(words_filtered) >= num:
                return ", ".join(words_filtered)
            else:
                print(f"You must enter a total of at least {num} words for this response.")
        except AttributeError:
            print("Please enter valid words.")


def nearest_half_hour_check(question):
    while True:
        try:
            value = float(input(question+" "))
            if (value%1 == 0 or value%1 == 0.5) and value>=0:
                return value
            else:
                print(f"Please enter a valid number rounded to the nearest half hour.")
        except ValueError:
...
type_of_exercise = ["Gym", "Walking", "Running", "Cycling", "Yoga", "Other"]
...
def get_responses_for(item):
    ...
    if item == "Exercise":
        ...
        for i in range(len(exercise_questions)):
            ...
            if i == 0:
                ans = mcq_and_rating_scale(exercise_questions[i], 1, 6)#.lower().strip()
                ans = type_of_exercise[ans-1]
                if ans == "Gym":
        ...
        for q in diet_questions:
            if q.startswith("Q1)"):
                ...
                meals_num = int(digit_check(q))
                if meals_num == 1:
                    ans_2_1 = yes_or_no("Q2.1) Was your meal today home-cooked? [Y: Yes/N: No]")
                    ans_2_2 = mcq_and_rating_scale(
                        """Q2.2) Rate the healthiness of the meal.\n[Rating Scale: 1 (Much Less Healthy) to 5 (Much Healthier)])""",
                        1, 5)
                    ans = f"{meals_num}, {ans_2_1}, {ans_2_2}"
                elif meals_num > 1:
                    ans_2_1 = mcq_and_rating_scale(
                        f"Q2.1) How many of your {meals_num} meals today were home-cooked? [Enter number] ", 0,
                        meals_num)
                    if int(ans_2_1) != meals_num and int(ans_2_1):
                        ans_2_2 = mcq_and_rating_scale(
                            """Q2.2) Rate the healthiness of meals eaten outside versus home-cooked.\n[Rating Scale: 1 (Much Less Healthy) to 5 (Much Healthier)])""",
                            1, 5)
                    else:
                        ans_2_2 = mcq_and_rating_scale(
                            """Q2.2) Rate the healthiness of the meal.\n[Rating Scale: 1 (Much Less Healthy) to 5 (Much Healthier)])""",
        ...
        for q in stress_level_questions:
            ...
            if q.startswith("Q2)"):
                ...
                ans = check_count_on_word(q, 2)
                if "Other" in ans:
                    index_other = ans.index("Other")
                    other_ans = check_count_on_word(f"""Please specify what "Other" entails:""", 1)
                    ans = ans[:index_other] + f"Other: {other_ans}" + ans[index_other + len("Other"):]
...
members_file = "members.json"  # Member data stored in file

if not add_member(members_file, user_name, password):
    if verify_member(members_file, user_name, password):
